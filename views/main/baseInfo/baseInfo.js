...
define(function(require) {
    ...
    app.controller('baseInfoCrl', ['$scope', '$rootScope', 'url', '$http', '$location', function($scope, $rootScope, url, $http, $location) {
        ...
        $scope.save = function() {
            ...
            if (!info.name.state) {
                yMake.layer.msg(info.name.info + '公司名称', {
                    icon: '2',
                    time: 2000
                });
                return;
            } else if (!info.intro.state) { //公司简介
                yMake.layer.msg(info.repeatPwd.info + '公司简介', {
                    icon: '2',
                    time: 2000
                });
                return;
            } else if (!info.corporation.state) {
                yMake.layer.msg(info.corporation.info + '公司法人', {
                    icon: '2',
                    time: 2000
                });
                return;
            } else if (!info.phone.state) {
                yMake.layer.msg(info.phone.info + '联系方式', {
                    icon: '2',
                    time: 2000
                });
                return;
            } else if (!info.phone1.state) {
                yMake.layer.msg(info.phone1.info, {
                    icon: '2',
                    time: 2000
                });
                return;
            } else if (!info.email.state) {
                yMake.layer.msg(info.email.info + '邮箱', {
                    icon: '2',
                    time: 2000
                });
                return;
            } else if (!info.email1.state) {
                yMake.layer.msg(info.email1.info, {
                    icon: '2',
                    time: 2000
                });
                return;
            } else if (!info.address.state) {
                yMake.layer.msg(info.address.info + '公司地址', {
                    icon: '2',
                    time: 2000
                });
                return;
            ...
            var urls1 = [],
                head1 = getImgSrc('#head1'),
                head2 = getImgSrc('#head2');
        ...
        function loadTrunkLine() {
            var fetchFunction = function(page, callback) {
                $http.post(url + '/dryline/showPageList?loginname=' + userInfo.data.loginname, $.extend({}, page, {
                    type: 0
                })).success(callback)
            };
        ...
        function loadkhrequest() {
            ...
            $scope.khrequests = {};
            var fetchFunction = function(page, callback) {
                $http.post(url + '/khrequest/showPageList', $.extend({}, page, {
