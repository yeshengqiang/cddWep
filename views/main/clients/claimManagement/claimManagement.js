    ...
    app.controller('claimManagementCrl', ['$scope', '$http', 'url', function($scope, $http, url) {
        ...
        $scope.changeState = function(item) {
            ...
            layer.confirm("是否修改索赔状态为" + info + "？", {
                ...
                $http.post(url + '/claim/changeState', claim).success(function() {
                    yMake.layer.msg("修改成功! ", {
                        icon: 1,
                        time: 1000
                    });
                    $scope.projectItem._load();
                }).error(function() {
                    yMake.layer.msg("修改失败! ", {
                        icon: 2,
                        time: 1000
                    });
                });
        ...
        $scope.outMessage = function() {
            ...
            layer.confirm("是否导出数据？", {
                ...
                window.location.href = url + "/claim/export1?loginname=" + userInfo.data.loginname + '&starttime=' + param.starttime + '&endtime=' + param.endtime + '&type=' + param.type;
        ...
        $scope.dropdownProvince = ['北京', '湖北', '浙江', '江苏'];
